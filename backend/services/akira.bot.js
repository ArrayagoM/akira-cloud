...
// Módulo instanciable para multi-tenancy
'use strict';
...
function crearAkiraBot(config, dataDir, sessionDir) {
  ...
  const MP_WEBHOOK_SECRET         = config.MP_WEBHOOK_SECRET || '';
  ...
  // ── Paths ───────────────────────────────────────────────────
  const CACHE_PATH               = path.join(dataDir, '_cache_temporal.json');
  const RESERVAS_PENDIENTES_PATH = path.join(dataDir, '_reservas_pendientes.json');
  ...
  const reservasPendientes    = cargar(RESERVAS_PENDIENTES_PATH);
  const recordatoriosActivos  = cargar(RECORDATORIOS_PATH);
  const slotsEnProceso        = new Set();
  const timeoutsRecordatorios = {};
  ...
  if (fs.existsSync(CREDENTIALS_PATH)) {
    try {
      ...
      log('⚠️ Error Google credentials: ' + e.message);
    }
  }
  ...
  function esNombreValido(t) {
    ...
    const inv = ['hola','si','no','ok','bien','dale','buenas','hey','test','ahi','nada'];
    return l.length >= 2 && !/^\d+$/.test(l) && !inv.includes(l.toLowerCase()) && /\p{L}/u.test(l);
  }
  ...
  async function obtenerEventos(calId, ini, fin) {
    ...
    try {
      ...
      const r = await cal.events.list({ calendarId: calId, timeMin: ini.toISOString(), timeMax: fin.toISOString(), singleEvents: true, orderBy: 'startTime' });
      return r.data.items || [];
    } catch (e) { log('❌ Calendar: ' + e.message); return []; }
  }
  async function horariosLibres(fecha) {
    const [y,m,d] = fecha.split('-').map(Number);
    const ini = crearFecha(y,m,d,HORA_INICIO_DIA), fin = crearFecha(y,m,d,HORA_FIN_DIA);
    const ev = await obtenerEventos(CALENDAR_ID, ini, fin);
    const libres = [];
  ...
  async function crearEvento(calId, resumen, desc, ini, fin, email, tel) {
    ...
    try {
      const cal = google.calendar({ version: 'v3', auth: calendarAuth });
      const ev = { summary: resumen, description: desc + (tel?`\nTel: +${tel}`:'') + (email?`\nEmail: ${email}`:''), start: { dateTime: ini.toISOString(), timeZone: ZONA_HORARIA }, end: { dateTime: fin.toISOString(), timeZone: ZONA_HORARIA }, reminders: { useDefault: false, overrides: [{ method:'email',minutes:24*60},{ method:'popup',minutes:30}] } };
      const r = await cal.events.insert({ calendarId: calId, resource: ev, sendUpdates: 'none' });
      return r.data;
    } catch (e) { log('❌ Evento: ' + e.message); return null; }
  }
  async function eliminarEvento(calId, evId) {
    if (!calendarAuth) return false;
    try { await google.calendar({ version:'v3',auth:calendarAuth }).events.delete({ calendarId:calId,eventId:evId }); return true; }
    catch { return false; }
  }
  ...
  async function crearPago(chatId, nombre, fecha, hora, horaFin) {
    return new Promise((res, rej) => {
      ...
      const titulo=horaFin?`Turno ${fecha} ${hora}-${horaFin} — ${NEGOCIO}`:`Turno ${fecha} ${hora} — ${NEGOCIO}`;
      const body=JSON.stringify({ items:[{title:titulo,quantity:cant,unit_price:PRECIO_TURNO,currency_id:'ARS'}], payer:{name:nombre}, external_reference:`${chatId}|${fecha}|${hora}|${horaFin||hora}`, notification_url:NGROK_DOMAIN?webhookUrl:'', expires:true, expiration_date_from:new Date().toISOString(), expiration_date_to:new Date(Date.now()+30*60000).toISOString() });
      const req=https.request({ hostname:'api.mercadopago.com',path:'/checkout/preferences',method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${MP_ACCESS_TOKEN}`} },(r)=>{ let d=''; r.on('data',c=>d+=c); r.on('end',()=>{ try{const p=JSON.parse(d); if(p.init_point) res(p); else rej(new Error('MP sin init_point'));}catch(e){rej(e);} }); });
      req.on('error',rej); req.write(body); req.end();
    });
  }
  ...
  function guardarMemoria(chatId, datos) {
    fs.writeFileSync(path.join(dataDir,`${extraerNumero(chatId)}.json`),JSON.stringify(datos,null,2),'utf8');
  }
  ...
  function programarRecs(chatId,nombre,fecha,hora) {
    ...
    for (const r of RECS) {
      ...
      if (timeoutsRecordatorios[tk]) clearTimeout(timeoutsRecordatorios[tk]);
      timeoutsRecordatorios[tk]=setTimeout(async()=>{ try{await client.sendMessage(chatId,r.msg(nombre,hora));log(`[REC] ✅ ${r.label} → ${nombre}`);}catch(e){log(`[REC] ❌ ${e.message}`);}},delay);
    }
  }
  ...
  function limpiarExpiradas() {
    ...
    if (cambio) guardar(RESERVAS_PENDIENTES_PATH,reservasPendientes);
  }
  ...
  function herramientas() {
    return [
      ...
      {type:'function',function:{name:'agendar_turno',description:'SOLO llamar si: (1) se consultó disponibilidad, (2) cliente eligió día Y hora, (3) cliente confirmó con sí/dale/reservame. NUNCA llamar solo por consulta.',parameters:{type:'object',properties:{fecha:{type:'string'},hora:{type:'string'},hora_fin:{type:'string',description:'Solo si pide rango'}},required:['fecha','hora']}}},
  ...
  async function llamarGroq(msgs, conTools=true) {
    ...
    try { return await groq.chat.completions.create(opts); }
      ...
      if (err.status===429){
        ...
        const e=new Error('RATE_LIMIT');e.isRateLimit=true;throw e;
      }
      if (err.status===400&&err.message.includes('tool_use_failed')){
        try{return await groq.chat.completions.create(opts);}catch(e2){const er=new Error('TOOL_USE_FAILED');er.isToolUseFailed=true;throw er;}
      }
      throw err;
    }
  }
  ...
  async function handleMessage(msg) {
    ...
    if (esAudio){
      ...
      if (!tr){await client.sendMessage(chatId,'¡Ups! No pude entender el audio. ¿Me lo escribís? 🙏');return;}
      texto=tr;fueAudio=true;
    }

    const bodyLower=texto.toLowerCase().trim();
    log(`${fueAudio?'🎤':'📩'} [${chatId}]: ${texto}`);
    let usuario=cargarMemoria(chatId);

    if (msg.fromMe){await manejarComando(bodyLower,chatId,usuario);return;}
    ...
    try {
      ...
      if (cacheTemporal[chatId]?.esperandoEmail){await capturaEmail(chatId,texto,usuario,chat);return;}
      if (quiereConDueno(bodyLower)){
        usuario.silenciado=true;guardarMemoria(chatId,usuario);
        await client.sendMessage(chatId,`¡Dale, ${usuario.nombre}! Le aviso a ${MI_NOMBRE} para que te contacte. 🙌`);return;
      }

      // Detección de preselección de hora
      ...
      const contenido=fueAudio?`[mensaje de voz] ${texto}`:texto;
      usuario.historial.push({role:'user',content:contenido});
      usuario.historial=recortarHistorial(usuario.historial,20);

      const respuesta=await procesarConIA(chatId,usuario,chat);
      ...
      if (audio){const ok=await enviarComoAudio(chatId,respuesta);if(!ok)await client.sendMessage(chatId,respuesta);}
      else await client.sendMessage(chatId,respuesta);

      await chat.clearState();
    } catch(err){
      log('❌ ERROR: '+err.message);
      try{await client.sendMessage(chatId,`¡Ups! Tuve un problema. ¿Me repetís la consulta, ${cargarMemoria(chatId)?.nombre||''}? 🙏`);}catch{}
    }
  }

  // ─── COMANDOS MAESTROS ───────────────────────────────────────
  async function manejarComando(bodyLower,chatId,usuario) {
    if (bodyLower.includes('akira stop')){if(usuario){usuario.silenciado=true;guardarMemoria(chatId,usuario);}await client.sendMessage(chatId,'*(Akira apagada)*');return;}
    if (bodyLower.includes('akira reactivate')){if(usuario){usuario.silenciado=false;guardarMemoria(chatId,usuario);}await client.sendMessage(chatId,'*(Akira reactivada)*');return;}
    if (bodyLower.includes('akira status')){
      ...
      await client.sendMessage(chatId,`*Usuarios (${arch.length}):*\n${lineas.join('\n')}`);
    }
  }
  ...
  async function registrarNombre(chatId,texto,tel) {
    if (!cacheTemporal[chatId]){
      ...
      await client.sendMessage(chatId,`¡Hola! ✨ Soy Akira, asistente de ${MI_NOMBRE} — ${NEGOCIO}.\n\n¿Cuál es tu nombre? 😊`);return true;
    }
    if (cacheTemporal[chatId]?.esperandoNombre){
      if (!esNombreValido(texto.trim())){await client.sendMessage(chatId,'¿Me decís tu nombre real? (solo letras) 😊');return true;}
      ...
      await client.sendMessage(chatId,s);return true;
    }
    return false;
  }
  ...
  async function capturaEmail(chatId,texto,usuario) {
    if (!esEmailValido(texto.trim())){await client.sendMessage(chatId,'¡Ese email no parece válido! ¿Lo escribís de nuevo? (ej: nombre@gmail.com)');return;}
  ...
  function quiereConDueno(t){
    return [`hablar con ${MI_NOMBRE.toLowerCase()}`,'pasame con','quiero hablar con','necesito hablar con','comunicame con'].some(f=>t.includes(f));
  }
  ...
  async function procesarConIA(chatId,usuario) {
    ...
    const sys={role:'system',content:
      ...
      `Hoy: ${fStr} | ISO: ${fISO}\nPróx días: ${prox}\n`+
      `Negocio: ${SERVICIOS} | Precio: $${PRECIO_TURNO} ARS/h | Cancelar/reagendar: mín ${HORAS_MINIMAS_CANCELACION}h anticipación.\n`+
      (pend?`🚨 PAGO PENDIENTE: ${pend.fecha} ${pend.hora}${pend.horaFin?'–'+pend.horaFin:''} ($${pend.totalPrecio||PRECIO_TURNO}). NO agendar otro.\n`:'')+
      ...
      `FLUJO RESERVA: 1.Consultar disponibilidad → 2.Cliente elige → 3.Confirmar → 4.Llamar agendar_turno.\n`+
      `NUNCA llamar agendar_turno sin confirmación explícita.\n`+
      `Max 4 líneas. Sin JSON/código. Cancelar→cancelar_turno, Cambiar→reagendar_turno.\n`+
      (PROMPT_EXTRA?`INSTRUCCIONES EXTRA DEL NEGOCIO: ${PROMPT_EXTRA}\n`:'')
    };
    ...
    if (msg.tool_calls?.length>0){
      ...
      for (const t of msg.tool_calls){
        ...
        log(`🔧 Tool: ${t.function.name} ${JSON.stringify(args)}`);
      ...
      const msgs2=[{role:'system',content:`Sos Akira de ${MI_NOMBRE}. Natural, cálido, WhatsApp con ${usuario.nombre}. Max 3 líneas.`+(linkMP?' El link de pago ya se agrega automáticamente — NO lo menciones.':'')}, ...usuario.historial];
  ...
  async function ejecutarTool(tool,args,chatId,usuario) {
    ...
    if (tool.function.name==='agendar_turno'){
      ...
      if (!confirma&&!porCache){push(`El cliente no confirmó aún. Preguntale si quiere el turno del ${args.fecha} a las ${args.hora} y esperá su respuesta.`);return;}
      if (cacheTemporal[chatId]?.preseleccionado){delete cacheTemporal[chatId].preseleccionado;guardar(CACHE_PATH,cacheTemporal);}
      limpiarExpiradas();
      const pend=pendienteActual(chatId);
      if (pend){push(`Ya tenés una reserva pendiente de pago para el ${pend.fecha} ${pend.hora}. Pagá esa primero.`);return;}
      ...
      try {
        ...
        if (conflictos.length>0){push(`El horario ${args.hora}–${hFn}:00 ya está ocupado. Pedile otro horario.`);return;}
        if (!usuario.email){
          ...
          push(`Para reservar el ${args.fecha} de ${args.hora} a ${hFn}:00 ($${total} ARS), necesitamos el email del cliente. Pedíselo.`);return;
        }
        ...
        guardar(RESERVAS_PENDIENTES_PATH,reservasPendientes);
        push(`Link generado. Reserva: ${args.fecha} ${args.hora}–${hFn}:00 (${cant} turno/s) $${total} ARS. Link: ${pref.init_point}. Solo se agenda si paga. Vence en 30 min.`);
      }catch(e){log('[MP] '+e.message);push(`Error generando link de pago: ${e.message}. Avisale que intente en unos minutos.`);}
    ...
    if (tool.function.name==='cancelar_turno'){
      ...
      if (!t){push(`No encontré turno confirmado para ${args.fecha} ${args.hora}.`);return;}
      ...
      if (hs<HORAS_MINIMAS_CANCELACION){push(`No se puede cancelar: faltan solo ${Math.round(hs)}hs. Contactar a ${MI_NOMBRE}.`);return;}
      ...
      guardarMemoria(chatId,usuario);
      push(`Turno ${args.fecha} ${args.hora}${t.horaFin?'–'+t.horaFin:''} cancelado. Slot liberado. Si hubo pago, ${MI_NOMBRE} gestionará el reembolso.`);return;
    }
    ...
    if (tool.function.name==='reagendar_turno'){
      ...
      if (hs<HORAS_MINIMAS_CANCELACION){push(`No se puede reagendar: faltan solo ${Math.round(hs)}hs. Contactar a ${MI_NOMBRE}.`);return;}
      ...
      if (!nuevo){push(`Error creando nuevo evento. Contactar a ${MI_NOMBRE}.`);return;}
  ...
  async function generarPago(chatId,usuario,fecha,hora,horaFin=null){
    try{
      ...
      if(pend){await client.sendMessage(chatId,`Ojo, ${usuario.nombre}! Ya tenés un turno pendiente de pago para el *${pend.fecha}* a las *${pend.hora}*.\n\nPagá ese primero. 💳`);return;}
      ...
      await client.sendMessage(chatId,`¡Perfecto! 🎉 Para confirmar tu turno del *${fecha}* ${r}:\n\n💳 *Pagá aquí:*\n${pref.init_point}\n\n💰 *$${total} ARS*${cant>1?` (${cant} turnos × $${PRECIO_TURNO})`:''}.\n⏳ Vence en 30 minutos.\n\nUna vez que pagues, el turno queda confirmado automáticamente. ✅`);
    }catch(e){log('[MP] Error pago: '+e.message);await client.sendMessage(chatId,'¡Ups! Error generando el link de pago. Intentá en unos minutos. 🙏');}
  }
  ...
  function iniciarServidor(){
    ...
    app.post('/webhook',async(req,res)=>{
      ...
      try{
        ...
        const rk=pago.external_reference,res=reservasPendientes[rk];
        if(!res)return;
        if(Date.now()>res.expiresAt){await client.sendMessage(res.chatId,`Hola ${res.nombre}! Tu pago fue recibido ✅ pero la reserva expiró. ${MI_NOMBRE} te contacta para reagendar. Se gestiona el reembolso. 🙏`);delete reservasPendientes[rk];guardar(RESERVAS_PENDIENTES_PATH,reservasPendientes);return;}
        const [y,m,d]=res.fecha.split('-').map(Number);
        const hI=parseInt(res.hora.split(':')[0]),hF=res.horaFin?parseInt(res.horaFin.split(':')[0]):hI+DURACION_RESERVA_HORAS;
        const ini=crearFecha(y,m,d,hI),fin=crearFecha(y,m,d,hF);
        const conf=await obtenerEventos(CALENDAR_ID,ini,fin);
        if(conf.length>0){await client.sendMessage(res.chatId,`Hola ${res.nombre}! Tu pago fue recibido ✅ pero el horario quedó ocupado. ${MI_NOMBRE} te contacta para reagendar o devolverte el dinero. 🙏`);delete reservasPendientes[rk];guardar(RESERVAS_PENDIENTES_PATH,reservasPendientes);return;}
        const um=cargarMemoria(res.chatId),tel=um?.numeroReal||extraerNumero(res.chatId);
        const ev=await crearEvento(CALENDAR_ID,`Turno — ${res.nombre}`,`WhatsApp: +${tel} | Pago MP ID: ${pago.id} | $${res.total||PRECIO_TURNO}`,ini,fin,res.email||null,tel);
        delete reservasPendientes[rk];guardar(RESERVAS_PENDIENTES_PATH,reservasPendientes);
        if(ev){
          if(um){if(!um.turnosConfirmados)um.turnosConfirmados=[];um.turnosConfirmados.push({fecha:res.fecha,hora:res.hora,horaFin:res.horaFin||null,pagoId:pago.id,confirmadoEn:new Date().toISOString()});um.historial.push({role:'assistant',content:`[SISTEMA] Pago MP confirmado (ID:${pago.id}). Turno ${res.fecha} ${res.hora}. YA PAGÓ.`});guardarMemoria(res.chatId,um);}
          programarRecs(res.chatId,res.nombre,res.fecha,res.hora);
          await client.sendMessage(res.chatId,`¡Listo, ${res.nombre}! 🎉\n✅ *Pago: $${res.total||PRECIO_TURNO} ARS*\n📅 *Turno:* ${res.fecha} de *${res.hora}${res.horaFin?'–'+res.horaFin:''}*\n${ev.htmlLink?`📆 ${ev.htmlLink}\n`:''}\n⏰ Te recordamos 24hs, 4hs y 30min antes. ¡Te esperamos! 🙌`);
        }else await client.sendMessage(res.chatId,`Hola ${res.nombre}! Pago recibido ✅ pero error en el calendario. ${MI_NOMBRE} confirma manualmente. 🙏`);
      }catch(e){log('[Webhook] '+e.message);}
    });
    app.get('/health',(_,r)=>r.json({ok:true}));
    expressServer=app.listen(PUERTO,()=>log(`🚀 Webhook en puerto ${PUERTO}`));
  }

  // ─── NGROK ───────────────────────────────────────────────────
  async function iniciarNgrok(){
    if(!NGROK_AUTH_TOKEN)return;
    try{const ng=require('@ngrok/ngrok');const l=await ng.forward({addr:PUERTO,authtoken:NGROK_AUTH_TOKEN,domain:NGROK_DOMAIN||undefined});log(`✅ Ngrok: ${l.url()}/webhook`);}
  ...
  async function iniciar(){
    ...
    client=new Client({
      authStrategy:new LocalAuth({dataPath:path.join(sessionDir,'wwebjs_auth')}),
      puppeteer:{headless:true,args:['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage','--no-zygote','--disable-gpu']},
    });

    client.on('qr',(qr)=>{log('📱 QR generado');emitter.emit('qr',qr);});
    client.on('ready',()=>{log('✅ WhatsApp conectado');emitter.emit('ready');reprogramarRecs();});
  ...
  async function detener(){
    for (const k of Object.keys(timeoutsRecordatorios)) clearTimeout(timeoutsRecordatorios[k]);
